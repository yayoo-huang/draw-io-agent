/**
 * Context management prompts for handling context window overflow
 * Based on Cline's context management system
 */
...
/**
 * Generates the prompt for summarizing the current task when context is full
 * Directly adapted from Cline's contextManagement.ts
 */
...
/**
 * Generates the continuation prompt that includes the summary
 * Directly adapted from Cline's continuationPrompt function
 */
export function continuationPrompt(summaryText: string): string {
	return `
