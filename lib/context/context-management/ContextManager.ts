...
import type { ApiHandler, ApiReqInfo, ConversationMessage } from "../types"
import { GlobalFileNames } from "../types"
...
export class ContextManager {
	...
	shouldCompactContextWindow(
		clineMessages: ConversationMessage[],
		api: ApiHandler,
		...
		if (previousApiReqIndex >= 0) {
			...
			if (previousRequest && previousRequest.text) {
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: ApiReqInfo = JSON.parse(previousRequest.text)
				...
				console.log(`🔍 Context Check: ${totalTokens} tokens used, threshold: ${thresholdTokens}, contextWindow: ${contextWindow}, maxAllowed: ${maxAllowedSize}`)
	...
	getContextTelemetryData(
		...
		if (targetIndex >= 0) {
			...
			if (targetRequest && targetRequest.text) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: ApiReqInfo = JSON.parse(targetRequest.text)
